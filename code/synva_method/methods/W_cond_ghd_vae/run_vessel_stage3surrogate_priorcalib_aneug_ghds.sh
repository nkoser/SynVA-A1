#!/usr/bin/env bash
# W+Vessel without morphology conditioning, but with the same prior-path
# calibration as the final morph_priorcalib model. This is the clean ablation:
# it isolates the contribution of morphology conditioning from the contribution
...
SAVE_ROOT_W=${SAVE_ROOT_W:-checkpoints/methods_aneug_ghds_refstage1/W_vessel_stage3surrogate_priorcalib}
META=${META:-W_vessel_stage3surrogate_priorcalib_seed${SEED}_$(date +%Y%m%d_%H%M%S)}
LOG=logs/${META}.log

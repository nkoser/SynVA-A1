#!/usr/bin/env bash
# W+Vessel v4: official-style Stage-1 loss + differentiable Step3 surrogate.
# The surrogate ring-fits the generated pouch to the target OPA ring and then
# optimizes label2-to-pouch, pouch center, side-of-ostium, and opening center.
set -euo pipefail
...
SAVE_ROOT_W=${SAVE_ROOT_W:-checkpoints/methods_aneug_ghds_refstage1/W_vessel_stage3surrogate}
META=${META:-W_vessel_stage3surrogate_seed${SEED}_$(date +%Y%m%d_%H%M%S)}
LOG=logs/${META}.log

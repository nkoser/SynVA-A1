#!/usr/bin/env bash
# W+Vessel v5: Step3 surrogate plus a nearest/neck-distance term.
# Goal: keep Wstage3's pouch-center/label2 gains while recovering Wvessel_v2's
# strong nearest_vertex_to_ostium behavior.
set -euo pipefail
...
SAVE_ROOT_W=${SAVE_ROOT_W:-checkpoints/methods_aneug_ghds_refstage1/W_vessel_stage3nearest}
META=${META:-W_vessel_stage3nearest_seed${SEED}_$(date +%Y%m%d_%H%M%S)}
LOG=logs/${META}.log

#!/usr/bin/env bash
# W+Vessel v3: official-style Stage-1 loss + explicit pouch placement loss.
# The pouch loss supervises the generated sac center offset/direction relative
# to its own opening center, so it does not use any inference-time target leak.
set -euo pipefail
...
W_SCALE=${W_SCALE:-1.0}
W_POUCH_OFFSET=${W_POUCH_OFFSET:-25.0}
W_POUCH_AXIS=${W_POUCH_AXIS:-0.25}
...
SAVE_ROOT_W=${SAVE_ROOT_W:-checkpoints/methods_aneug_ghds_refstage1/W_vessel_pouchloss}
META=${META:-W_vessel_pouchloss_seed${SEED}_$(date +%Y%m%d_%H%M%S)}
LOG=logs/${META}.log
...
conda run --no-capture-output -n unified_env python methods/W_cond_ghd_vae/train.py \
  ...
  --w_ring 0.0 \
  --w_ring_chamfer 0.0 \
  --w_pouch_offset "${W_POUCH_OFFSET}" \
  --w_pouch_axis "${W_POUCH_AXIS}" \

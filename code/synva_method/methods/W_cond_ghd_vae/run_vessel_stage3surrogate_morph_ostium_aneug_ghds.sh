#!/usr/bin/env bash
# W+Vessel+Morphology: official-style Stage-1 loss + Step3 surrogate, with
# only the selected ostium/pouch morphology scalars appended to the condition.
set -euo pipefail
...
MORPHOLOGY_KEYS=${MORPHOLOGY_KEYS:-A_A,V_A,A_CH,V_CH,D_max,H_max,W_max,H_ortho,W_ortho}
...
W_SHAPE_EXTENT=${W_SHAPE_EXTENT:-0.0}
W_SHAPE_AREA=${W_SHAPE_AREA:-0.0}
W_SHAPE_VOLUME=${W_SHAPE_VOLUME:-0.0}
W_SHAPE_MOMENT=${W_SHAPE_MOMENT:-0.0}
W_SURFACE_CHAMFER=${W_SURFACE_CHAMFER:-0.0}
W_SURFACE_NORMAL=${W_SURFACE_NORMAL:-0.0}
SURFACE_SAMPLES=${SURFACE_SAMPLES:-2048}
...
SAVE_ROOT_W=${SAVE_ROOT_W:-checkpoints/methods_aneug_ghds_refstage1/W_vessel_stage3surrogate_morph_ostium}
META=${META:-W_vessel_stage3surrogate_morph_ostium_seed${SEED}_$(date +%Y%m%d_%H%M%S)}
LOG=logs/${META}.log
...
conda run --no-capture-output -n unified_env python methods/W_cond_ghd_vae/train.py \
  ...
  --w_shape_extent "${W_SHAPE_EXTENT}" \
  --w_shape_area "${W_SHAPE_AREA}" \
  --w_shape_volume "${W_SHAPE_VOLUME}" \
  --w_shape_moment "${W_SHAPE_MOMENT}" \
  --w_surface_chamfer "${W_SURFACE_CHAMFER}" \
  --w_surface_normal "${W_SURFACE_NORMAL}" \

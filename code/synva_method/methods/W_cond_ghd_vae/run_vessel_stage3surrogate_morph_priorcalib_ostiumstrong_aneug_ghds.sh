#!/usr/bin/env bash
# W+Vessel+Morphology with prior-path calibration and stronger ostium/Stage-3
# supervision.  This is the follow-up to morph_priorcalib: keep the improved
# generative morphology distribution, but recover W_ref-level attachment metrics.
set -euo pipefail
...
W_SCALE=${W_SCALE:-1.0}
W_STAGE3_LABEL2=${W_STAGE3_LABEL2:-8.0}
W_STAGE3_CENTER=${W_STAGE3_CENTER:-12.0}
W_STAGE3_NEAREST=${W_STAGE3_NEAREST:-10.0}
W_STAGE3_SIDE=${W_STAGE3_SIDE:-4.0}
W_STAGE3_OPENING=${W_STAGE3_OPENING:-25.0}
...
SAVE_ROOT_W=${SAVE_ROOT_W:-checkpoints/methods_aneug_ghds_refstage1/W_vessel_stage3surrogate_morph_priorcalib_ostiumstrong}
META=${META:-W_vessel_stage3surrogate_morph_priorcalib_ostiumstrong_seed${SEED}_$(date +%Y%m%d_%H%M%S)}
LOG=logs/${META}.log
...
conda run --no-capture-output -n unified_env python methods/W_cond_ghd_vae/train.py \
  ...
  --w_stage3_nearest "${W_STAGE3_NEAREST}" \

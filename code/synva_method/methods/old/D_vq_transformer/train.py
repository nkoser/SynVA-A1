...
def stage1(args, device, log):
    ...
    for ep in range(args.stage1_epochs):
        ...
        for batch in train_dl:
            ...
            loss = mse + args.commitment_beta * commitment
            opt.zero_grad(); loss.backward()
            torch.nn.utils.clip_grad_norm_(list(model.parameters()) + list(conditioner.parameters()), 1.0)
            opt.step()
            tr_mse.append(mse.item()); tr_cm.append(commitment.item()); tr_perp.append(perp.item())
        ...
        if (ep + 1) % 25 == 0 or ep == args.stage1_epochs - 1:
            ...
            vmean = sum(vl)/len(vl)
            log(f"[S1] ep {ep+1:4d} train_mse {sum(tr_mse)/len(tr_mse):.4f} "
...
def main():
    ...
    payload = {
        ...
        "stage1_val_mse": best1["val_mse"], "stage2_val_ce": best2["val_ce"],
    }

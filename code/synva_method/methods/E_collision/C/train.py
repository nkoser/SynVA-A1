"""Method E-C: FSQ-VAE + AR Prior with vessel-collision loss in stage 1.

Forks methods/C_fsq_ar/train.py.  Re-uses the existing FSQVAE / FSQARPrior;
only adds the collision loss term in stage 1.
"""
...
def stage1(args, device, log):
    ...
    if opening_idx is not None and args.exclude_ring_from_collision:
        ...
        log(f"[S1] sac_mask: V={V}, opening_ring={int((~sac_mask).sum())}, sac={int(sac_mask.sum())}")
    else:
        sac_mask = None
        log(f"[S1] sac_mask: using ALL {V} verts")
    collision = VesselCollisionLoss(
        clearance=args.collision_clearance, sac_mask=sac_mask,
        chunk=args.collision_chunk,
    ).to(device)

    ghd_mean = train_ds.ghd_mean.to(device); ghd_std = train_ds.ghd_std.to(device)
    vessel_center = train_ds.vessel_center.to(device)
    vessel_scale  = torch.as_tensor(train_ds.vessel_scale, device=device)
    def _denorm_vp(vp):
    ...
    for ep in range(args.stage1_epochs):
        ...
        tr_loss, tr_vert, tr_col = [], [], []
        viol_acc, viol_n = 0.0, 0
        ...
        if (ep + 1) % 25 == 0 or ep == args.stage1_epochs - 1:
            ...
            log(f"[S1] ep {ep+1:4d} train_loss {sum(tr_loss)/len(tr_loss):.4f} "
                ...
                f"col {sum(tr_col)/len(tr_col):.6f} (w={w_col:.2f}, viol_frac={viol_acc/max(1,viol_n):.3f}) "
                f"| val_vert {v_vert:.6f} val_col {v_col:.6f} val_viol {v_viol:.3f}")
            if v_vert < best_val:
                best_val = v_vert
                best = {
...
def main():
    ...
    log(f"=== Method E-C: FSQ-VAE + AR + Vessel Collision ===")

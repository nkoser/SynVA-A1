"""Method E-D: VQ-VAE + AR Prior with vessel-collision loss in stage 1.

Forks methods/D_vq_transformer/train.py.  Only stage-1 differs from D:
the recon loss gets an extra term

    L += w_collision(ep) * VesselCollisionLoss(pred_verts, vessel_pts)

evaluated in the SAME `ghd_local` frame.  Stage 2 (AR prior) is unchanged.

Re-uses the existing methods/D_vq_transformer model (no changes to it).
"""
...
from methods.E_collision.collision_loss import (
    VesselCollisionLoss, add_collision_args, collision_weight,
    build_sac_mask, load_opening_idx,
)
...
def stage1(args, device, log):
    ...
    V = coeff2mesh.canonical.verts_padded().shape[1]
    opening_idx = load_opening_idx(args.canonical_mesh_obj, args.opa_checkpoint)
    ring_loss_idx = opening_idx.to(device) if opening_idx is not None else None
    if opening_idx is not None and args.exclude_ring_from_collision:
        opening_idx = opening_idx.to(device)
        sac_mask = build_sac_mask(V, opening_idx)
        log(f"[S1] sac_mask: V={V}, opening_ring={int((~sac_mask).sum())}, "
            f"sac={int(sac_mask.sum())}")
    else:
        sac_mask = None
        log(f"[S1] sac_mask: using ALL {V} verts (opening_idx not found or disabled)")

    collision = VesselCollisionLoss(
        clearance=args.collision_clearance,
        sac_mask=sac_mask,
        chunk=args.collision_chunk,
    ).to(device)
    ...
    # vessel_pts in the batch are standardised by the dataset; pred_verts come
    # out of CoeffToMesh in the unmodified canonical-mesh frame.  Push the
    # batch vessel_pts back to that frame before computing collision.
    vessel_center = train_ds.vessel_center.to(device)   # [1, 3]
    vessel_scale  = torch.as_tensor(train_ds.vessel_scale, device=device)  # scalar
    def _denorm_vp(vp):
        return vp * vessel_scale + vessel_center
    use_normals = (not args.no_normals) and (args.w_normal > 0)
    log(f"[S1] mesh-aware loss: w_mse={args.w_mse} w_vert={args.w_vert} w_normal={args.w_normal}")
    if ring_loss_idx is not None:
        log(f"[S1] opening-ring loss: idx={int(ring_loss_idx.numel())} "
            f"w_ring={args.w_ring} w_ring_chamfer={args.w_ring_chamfer}")
    else:
        log("[S1] opening-ring loss: disabled (no opening indices found)")
    log(f"[S1] collision: w={args.w_collision} clearance={args.collision_clearance} "
        f"phase_in={args.collision_phase_in} ramp={args.collision_ramp}")
    best_val = float("inf"); best = None; last_flat = None
    for ep in range(args.stage1_epochs):
        model.train(); conditioner.train()
        w_col = collision_weight(ep, args)
        tr_loss, tr_vert, tr_ring, tr_chamfer, tr_cm, tr_perp, tr_col = [], [], [], [], [], [], []
        col_stats_acc = {"min_dist_mean": 0.0, "min_dist_p05": 0.0, "violators_frac": 0.0, "n": 0}
        for batch in train_dl:
            ...
            if ring_loss_idx is not None and (args.w_ring > 0 or args.w_ring_chamfer > 0):
                ...
                ring_mse, ring_chamfer = opening_ring_losses(pred_verts, target_ring, ring_loss_idx)
            ...
            if w_col > 0.0:
                pred_verts, _ = coeff2mesh(recon, ghd_mean, ghd_std, want_normals=False)
                vessel_pts = _denorm_vp(batch["vessel_pts"].to(device))
                col_loss, col_stats = collision(pred_verts, vessel_pts, return_stats=True)
                loss = loss + w_col * col_loss
                col_loss_val = col_loss.item()
                for k in ("min_dist_mean", "min_dist_p05", "violators_frac"):
                    col_stats_acc[k] += col_stats[k]
                col_stats_acc["n"] += 1

            opt.zero_grad(); loss.backward()
            torch.nn.utils.clip_grad_norm_(list(model.parameters()) + list(conditioner.parameters()), 1.0)
            opt.step()
            tr_loss.append(loss.item()); tr_vert.append(vert_mse.item())
            tr_ring.append(ring_mse.item()); tr_chamfer.append(ring_chamfer.item())
            tr_cm.append(commitment.item()); tr_perp.append(perp.item())
            tr_col.append(col_loss_val)
        ...
        if (ep + 1) % 25 == 0 or ep == args.stage1_epochs - 1:
            ...
            with torch.no_grad():
                vl_vert, vl_ring, vl_chamfer, vl_col, vl_viol = [], [], [], [], []
                for batch in val_dl:
                    ghd = batch["ghd"].to(device)
                    cond = encode_cond(conditioner, batch, device,
                                       no_vessel_pts=args.no_vessel_pts,
                    ...
                    _, v_mse, _ = mesh_recon_losses(
                        coeff2mesh, ghd, recon, ghd_mean, ghd_std, want_normals=use_normals,
                    )
                    vl_vert.append(v_mse.item())
                    pred_verts, _ = coeff2mesh(recon, ghd_mean, ghd_std, want_normals=False)
                    ...
                    if ring_loss_idx is not None:
                        target_ring = batch["ostium_ring"].to(device).reshape(ghd.shape[0], -1)
                        target_ring = (target_ring * ring_std + ring_mean).view(ghd.shape[0], args.ring_points, 3)
                        r_mse, r_ch = opening_ring_losses(pred_verts, target_ring, ring_loss_idx)
                    vl_ring.append(r_mse.item()); vl_chamfer.append(r_ch.item())
                    vp = _denorm_vp(batch["vessel_pts"].to(device))
                    cl, cs = collision(pred_verts, vp, return_stats=True)
                    vl_col.append(cl.item()); vl_viol.append(cs["violators_frac"])
            v_vert = sum(vl_vert)/len(vl_vert)
            v_ring = sum(vl_ring)/len(vl_ring)
            v_chamfer = sum(vl_chamfer)/len(vl_chamfer)
            v_col  = sum(vl_col)/len(vl_col)
            v_viol = sum(vl_viol)/len(vl_viol)
            n = max(1, col_stats_acc["n"])
            log(f"[S1] ep {ep+1:4d} train_loss {sum(tr_loss)/len(tr_loss):.4f} "
                ...
                f"col {sum(tr_col)/len(tr_col):.6f} (w={w_col:.2f}, viol_frac={col_stats_acc['violators_frac']/n:.3f}) "
                f"| val_vert {v_vert:.6f} val_ring {v_ring:.6f} val_ring_ch {v_chamfer:.6f} "
                f"val_col {v_col:.6f} val_viol {v_viol:.3f}")
            if v_vert < best_val:
                best_val = v_vert
                best = {
                    "model": {k: v.detach().cpu().clone() for k, v in model.state_dict().items()},
                    "conditioner": {k: v.detach().cpu().clone() for k, v in conditioner.state_dict().items()},
                    "epoch": ep + 1, "val_vert": v_vert,
                    "val_ring": v_ring, "val_ring_chamfer": v_chamfer,
                    "val_col": v_col, "val_viol": v_viol,
                }
...
def main():
    ...
    log(f"=== Method E-D: VQ-VAE + AR + Vessel Collision ===")
    ...
    log(f"[S1] done best_val_vert {best1['val_vert']:.5f} val_col {best1['val_col']:.6f} "
        f"val_viol {best1['val_viol']:.3f} ep {best1['epoch']} time {time.time()-t0:.0f}s")
    ...
    payload = {
        ...
        "stage1_val_col":  best1["val_col"],
        "stage1_val_viol": best1["val_viol"],

...
def _ghd_to_verts(c2m, ghd_normalized, ds):
    """ghd_normalized: [B, 432] in normalized space.
    Returns verts [B, V, 3] decoded against the *scaled* canonical mesh.
    """
...
def main():
    ...
    def _recon_mesh(ghd_norm_1xN, ds):
        v = _ghd_to_verts(c2m, ghd_norm_1xN, ds)[0].detach().cpu().numpy()
        return v, canonical_faces

    for ci, case in enumerate(cases):
        case_dir = os.path.join(args.out_dir, case)
        os.makedirs(case_dir, exist_ok=True)
        gv, gf = _recon_mesh(gt_norm0[ci:ci+1], ds0)

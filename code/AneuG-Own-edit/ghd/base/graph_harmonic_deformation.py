...
class Graph_Harmonic_Deform(nn.Module):
    def __init__(self, base_shape: Meshes, num_Basis = 6*6+1, mix_lap_weight= [1,0.1,0.1], eigen_chk=None):
        ...
        self.mix_lap = mix_lap_weight[0]*self.cotlap_np + mix_lap_weight[1]*self.norlap_np + mix_lap_weight[2]*self.standerlap_np # use most of the cotlap_np and use norlap_np and standerlap_np as normalization
        if eigen_chk is not None:
            ...
            self.GBH_eigval, self.GBH_eigvec = eigsh(self.mix_lap, k=num_Basis, which='SM') # k -> Only give me the k smoothest, simplest patterns. Throw away the complex, jittery ones.
        ...
        self.deformation_param = nn.Parameter(torch.zeros((num_Basis, 3), dtype=torch.float32, device=base_shape.device), requires_grad=True) # learnable parameters result = Base + (w_1 * Eigenvector_1) + (w_2 * Eigenvector_2) + (w_3 * Eigenvector_3)
...
class Graph_Harmonic_Deform_opening_alignment_dynamic(Graph_Harmonic_Deform):
    def __init__(self, args, oa_class: RegistrationwOpeningAlignment, eigen_chk=None):
        ...
        for attr_name in ["op_rec_v", "op_rec_v_indices_map", "op_rec_f", "op_rec_f_map"]:
        ...
        for idx in range(self.num_op):
            op_verts = torch.as_tensor(self.op_rec_v[idx], dtype=torch.float32, device=self.device)
            op_faces = torch.as_tensor(self.op_rec_f[idx], dtype=torch.int64, device=self.device)
            self.open_Meshes.append(Meshes(verts=[op_verts], faces=[op_faces]))
    ...
    def forward_with_opening_alignment(self, GHB_coefficient=None):
        ...
        output_openings = []
        for idx in range(self.num_op):
            mapped_ids = torch.as_tensor(self.op_rec_v_indices_map[idx], dtype=torch.int64, device=self.device)
            output_openings.append(self.open_Meshes[idx].offset_verts(deformation[mapped_ids, :]))
        R_matrix = axis_angle_to_matrix(self.R)
        output_shape = output_shape.update_padded((output_shape.verts_padded() @ R_matrix.transpose(-1,-2)*self.s.abs() + self.T).float())
        output_openings = [
            op_mesh.update_padded((op_mesh.verts_padded() @ R_matrix.transpose(-1, -2) * self.s.abs() + self.T).float())
            for op_mesh in output_openings
        ]

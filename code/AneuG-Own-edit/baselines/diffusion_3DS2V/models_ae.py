...
def create_autoencoder(dim=512, M=512, latent_dim=64, N=2048, depth=6, determinisitc=False):
    if determinisitc:
        model = AutoEncoder(
            depth=depth,
            dim=dim,
...
def get_vae(dim=512, M=512, latent_dim=512, N=2048, depth=6, determinisitc=False):
    return create_autoencoder(dim=dim, M=M, latent_dim=latent_dim, N=N, depth=depth, determinisitc=determinisitc)

def kl_d512_m512_l512_d6(N=2048):
    return create_autoencoder(dim=512, M=512, latent_dim=512, N=N, depth=6, determinisitc=False)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        """
        preds: Predicted probabilities or logits (B, N)
        targets: Ground truth binary mask (B, N)
        """
        # Convert predictions to probabilities (if not already in [0, 1])

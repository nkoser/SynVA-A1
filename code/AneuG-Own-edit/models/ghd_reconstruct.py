...
class GHD_Reconstruct(object):
    ...
    * For forward_as_Meshes, denormalize_shape will return shape as the state just after alignment
    ...
    def __init__(self, canonical_Meshes: Meshes,
                 ...
                 norm_canonical_override=None):
        self.canonical_Meshes = canonical_Meshes.to(device)
        if skip_normalize:
            # Mesh is already normalized (e.g., by GHD fitting normalization).
            # norm_canonical is used by denormalize_shape to map back to raw space.
            if norm_canonical_override is not None:
                self.norm_canonical = norm_canonical_override
            else:
                self.norm_canonical = 1.0
        else:

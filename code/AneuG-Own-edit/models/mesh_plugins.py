...
class MeshRegulizer(object):  
    ...
    def __init__(self, 
        ...
        self.run_shapiro_test = bool(run_shapiro_test)
    ...
    def KL_regulization(self, meshes: Meshes, real_meshes: Meshes, MEA=False, overreg=False):
        ...
        """
        # Pytorch3D sparse laplacian ops do not support bfloat16 on CUDA.
        # Force FP32 verts for all regularization terms while preserving gradients.
        if meshes.verts_padded().dtype != torch.float32:
            meshes = meshes.update_padded(meshes.verts_padded().float())
        if real_meshes.verts_padded().dtype != torch.float32:
            real_meshes = real_meshes.update_padded(real_meshes.verts_padded().float())

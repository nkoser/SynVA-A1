...
class ResidualBlock(nn.Module):
    def __init__(self, in_features, out_features, norm_type="batch"):
...
def build_norm_1d(num_features, norm_type="batch"):
...
class VAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, withscale=False, norm_type="batch"):
...
class ConditionalGHDVAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, cond_dim=2, cond_embed_dim=None, norm_type="batch"):
    ...
    def forward(self, x, cond, z=None, decode_only=False, noise_scale=1.0):
        if decode_only:
            if z is None:
                raise ValueError("z must be provided when decode_only=True")
            return self.decode(z, cond)
        mu, logvar = self.encode(x)
        z = self.reparameterize(mu, logvar, noise_scale=noise_scale)
        return self.decode(z, cond), mu, logvar

    def forward_encoder(self, x, noise_scale=1.0):
        mu, logvar = self.encode(x)
        z = self.reparameterize(mu, logvar, noise_scale=noise_scale)
        return z, mu, logvar
